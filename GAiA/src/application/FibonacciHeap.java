...
/**
 * A class representing a Fibonacci heap used in DijkstraAlgorithm class. User
 * Note: This class has been entirely imported from the author mentioned below.
 * Some commentary of the code has been removed.
 *
 * @param T
 *          The type of elements to store in the heap.
 * @author Keith Schwarz (htiek@cs.stanford.edu) described by Fredman and Tarjan.
 */
public class FibonacciHeap<T> {
  ...
  public static <T> FibonacciHeap<T> merge(FibonacciHeap<T> one, FibonacciHeap<T> two) {
     ...
     * also computes the min of the two lists, so we can store the result in the
     * mMin field of the new heap.
     */
  ...
  public Entry<T> dequeueMin() {
    ...
    /*
     * Now, we need to get rid of this element from the list of roots. There are
     * two cases to consider. First, if this is the only element in the list of
     * roots, we set the list of roots to be null by clearing mMin. Otherwise,
     * if it's not null, then we write the elements next to the min element
     ...
     * they're about to become roots. Because the elements are stored in a
    ...
    /*
     * Next, splice the children of the root node into the topmost list, then
     * set mMin to point somewhere in that list.
     */
    ...
    /*
     * Next, we need to coalsce all of the roots so that there is only one tree
     * of each degree. To track trees of each size, we allocate an ArrayList
     * where the entry at position i is either null or the unique tree of degree
     * i.
     */
     ...
     * mid-stream. One major challenge is how to detect whether we're visiting
     * the same node twice. To do this, we'll spent a bit of overhead adding all
     * of the nodes to a list, and then will visit each element of this list in
     * order.
     */
    List<Entry<T>> toVisit = new ArrayList<Entry<T>>();

    /*
     * To add everything, we'll iterate across the elements until we find the
     * first element twice. We check this by looping while the list is empty or
     * while the current element isn't the first element of that list.
     */
    ...
    for (Entry<T> curr : toVisit) {
      ...
      while (true) {
        ...
        /*
         * If nothing's here, we're can record that this tree has this size and
         * are done processing.
         */
        ...
        /*
         * Determine which of the two trees has the smaller root, storing the
         * two tree accordingly.
         */
      ...
      /*
       * Update the global min based on this node. Note that we compare for <=
       * instead of < here. That's because if we just did a reparent operation
       * that merged two different trees of equal priority, we need to make sure
       * that the min pointer points to the root-level one.
       */
   ...
   *           If the new priority exceeds the old priority, or if the argument
   *           is not a finite double.
   */
   ...
   * lists, merges the two lists together into one circularly-linked list in
   * O(1) time. Because the lists may be empty, the return value is the only
  ...
  private static <T> Entry<T> mergeLists(Entry<T> one, Entry<T> two) {
    ...
    if (one == null && two == null) { // Both null, resulting list is null.
      ...
      Entry<T> oneNext = one.mNext; // Cache this since we're about to overwrite
                                    // it.
      one.mNext = two.mNext;
      one.mNext.mPrev = one;
      two.mNext = oneNext;
  ...
  private void decreaseKeyUnchecked(Entry<T> entry, double priority) {
     ...
     * that this also means that if we try to run a delete operation that
     * decreases the key to -infinity, it's guaranteed to cut the node from its
     * parent.
     */
    ...
    /*
     * If our new value is the new min, mark it as such. Note that if we ended
     * up decreasing the key in a way that ties the current minimum priority,
